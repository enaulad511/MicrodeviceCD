...
def run_experiment_read_loop(method, on_data=None):
    ...
    Termina por uno de cuatro caminos y avisa al host con un tipo distinto:
      ...
      - tope absoluto (MAX_EXPERIMENT_MS)    -> Z, drena limpio  -> emstat_maxtime
    ...
    while True:
        ...
        if time.ticks_diff(now_ms(), start) > MAX_EXPERIMENT_MS:
            _send_abort_to_emstat()
            clean = _drain_after_z(method, on_data)
        ...
        if line.lower().startswith(ERROR_TOKEN):
            ...
            if time.ticks_diff(now_ms(), last_data) > MAX_IDLE_MS:
        ...
        stripped = line.strip()
        if stripped == "":
            # Blank: solo es fin REAL si viene justo tras un '*'. Una blank sin '*'
            # previo es un separador entre meas_loops (preprocesamiento) -> se ignora.
            if last_was_star:
...
def handle_command(cmd_obj: dict):
    ...
    if cmd_obj.get("method") == "cv":
        ...
        # EIS: scan type Default + frequency Scan (Fase 1). Tiempos de
        # acondicionamiento en "0"/"" se normalizan a "" (etapa omitida).
